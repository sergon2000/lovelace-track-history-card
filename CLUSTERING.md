...
Every stop is shown at **street** level — `Street [number], City` (e.g. `Main St,
Madrid`) — so stops are always identifiable. When the response has no street
(`road`/`neighbourhood`/`suburb`) it falls back to `City`, and finally to the
country name alone.
