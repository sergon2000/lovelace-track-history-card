...
- Optional **reverse geocoding** (opt-in) labels stops outside any zone with their address (street and city)
...
| `reverse_geocode` | `boolean` | `false` | Look up an address (street and city) for stops that fall outside every Home Assistant zone. **Off by default** — see [Reverse geocoding](#reverse-geocoding). |
