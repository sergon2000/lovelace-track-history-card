...
- Optional **reverse geocoding** (opt-in) labels stops outside any zone with their address — city/country, or street level when several stops share the same city
...
| `reverse_geocode` | `boolean` | `false` | Look up an address (city/country, or street when several stops share a city) for stops that fall outside every Home Assistant zone. **Off by default** — see [Reverse geocoding](#reverse-geocoding). |
